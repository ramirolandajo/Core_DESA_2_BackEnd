...
public class KafkaMockService {
    ...
    public void handleAcknowledgement(EventAckEntity ack) {
        ...
        // EventAckEntity.eventId es un String (UUID) que representa el ID usado en EventMessage
        // LiveMessage.eventId es Integer (autogenerated DB id). Aquí intentamos parsear a Integer si es numérico.
        Optional<LiveMessage> liveOpt;
        try {
            Integer evtIdParsed = Integer.valueOf(ack.getEventId());
            liveOpt = liveMessageRepository.findByEventId(evtIdParsed);
        } catch (NumberFormatException nfe) {
            // si el eventId del ACK no es numérico (p.ej. UUID), no existe mapping a LiveMessage por integer id
            liveOpt = Optional.empty();
        }
        if (liveOpt.isEmpty()) {
